...
while True:
    ...
    if  event == 'расписание':
        write_msg(my_id, 'расписание для учителя', random)
    elif event == 'поменять расписание':
        write_msg(my_id, 'поменять расписание для одного учителя', random)
    elif event == 'выйти':
        break
    else:
        write_msg(my_id, 'не понимаю тебя', random)
    write_msg(my_id, 'расписание - узнать расписание\n'
                     'поменять расписание - поменять свой урок \n'
